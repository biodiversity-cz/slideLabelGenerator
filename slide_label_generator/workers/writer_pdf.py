...
def write_pdf(data: pd.DataFrame, path: str, margin_top: float = 10,
              ...
              margin_right: float = 10, padding: float = 1) -> None:
    """
    ...
    labels_per_column = int(available_height // label_height)
    
    # Create labels from DataFrame (excluding header)
    labels = []
    for _, row in data.iterrows():
        label = Label(row, label_width, label_height, padding * mm)
        labels.append(label)
    ...
    for page_start in range(0, len(labels), page_label_count):
        ...
        table = Table(grid_data, colWidths=label_width, rowHeights=label_height)
